...
| `TCPCLIENT_RX_BUFFER_LEN` | `100` | Command receive buffer size |
...
Most commands use a fixed response format:
...
Command `99` uses this fixed response to expose board diagnostics. Its storage-specific tail fields are:
...
The current implementation addresses the last two requirements in `tcpclient.c`:
...
- `lwipopts.h` TCP PCB override lives in `USER CODE BEGIN 1`.
