...
Offset calibration values are stored as six signed 32-bit channel offsets in CM4 firmware state. Firmware defaults are all zero. After command `98` runs, CM4 writes the offsets to `ocal.cfg` on eMMC and reloads that file after every successful eMMC mount, so the values survive a power cycle unless the calibration file is deleted or the eMMC filesystem is reformatted.

Calibration command behavior:
- command `97`: returns the currently loaded six offset values
- command `98`: configures the ADS path around 2000 Hz, arms the normal DRDY interrupt/SPI DMA capture path, ignores the first 5 seconds, accumulates channel sums for the next 10 seconds, divides each sum by the captured sample count, saves the six averages, and returns them
- subsequent `DAQ_Startup()` calls apply the latest values through `calibrate(calibration_values[channel], channel)`
...
For current board/host assumptions (`channel_mask = 0x3F`):
...
- if workflows use a channel mask beyond `0x3F`, the command `11` binary log stride changes immediately and host physical-value decoders must be updated
